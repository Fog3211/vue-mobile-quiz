  ...
  <div class="page">
    ...
    <div class="other-option">
      <router-link class="item" v-for="(item, index) in info_list" :key="index" :to="{ path: item.path }">
        <p class="item-title">
          <span :class="['myicon iconfont', item.icon]" :style="item.icon_color"></span>
  ...
  export default {
    ...
    activated() {
            Service.getProfile({
                ...
                if (res.code == 1) {
                   this.nickname=res.data.nickname?res.data.nickname:store.state.username;
                   this.introduction=res.data.introduction?res.data.introduction:"您还没有个性签名";
                }
            });
             Service.getAvatar({
                username: store.state.username,

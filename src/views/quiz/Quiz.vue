<template>
  <div class="page">
    ...
    >
      <div class="quiz">
        ...
        <ul class="quiz-options">
          <li
            ...
            @click="checkAnswer(index, items)"
            :class="['option', answer_state_list[index]]"
          >
      ...
      <div
        class="explain"
        v-show="index === current_index && reason_state == true"
      >
        <p><span class="title">最佳解释：</span>{{ items.reason }}</p>
      </div>
    </div>
    <!-- <div class="quiz-menu" v-if="quiz_menu">
  ...
  methods: {
    ...
    checkAnswer(index, items) {
      ...
      if (index == items.answer) {
        Vue.set(this.answer_state_list, index, "answer-yes");
      } else {
        Vue.set(this.answer_state_list, index, "answer-no");
        Vue.set(this.answer_state_list, items.answer, "answer-yes");
